...
@note If a valid path argument that points to an in model \ref SUEntityRef is
      supplied then the dimension will automatically be added to the
      \ref SUEntitiesRef that contains it. Otherwise a call to
      SUEntitiesAddDimension() will be required to add the newly created
      dimension to a model or component.
*/
