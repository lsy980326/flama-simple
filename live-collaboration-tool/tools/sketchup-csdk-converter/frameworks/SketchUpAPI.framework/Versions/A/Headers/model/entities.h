...
/**
@brief Adds dimension objects to an entities object.
@since SketchUp 2026.1, API 14.1
@param[in,out] entities_ref The entities object.
@param[in]     len          The length of the array of dimension objects.
@param[in]     dim_refs     The array of dimension objects to add.
@related SUEntitiesRef
@return
- \ref SU_ERROR_NONE on success
- \ref SU_ERROR_INVALID_INPUT if \p entities_ref is not a valid object
- \ref SU_ERROR_NULL_POINTER_INPUT if \p dim_refs is `NULL`
@note Some dimension creation and manipulation functions such as
      SUDimensionLinearCreate() and SUDimensionRadialCreate() may have already
      added the dimension to a \ref SUEntitiesRef. If that is the case users
      should take care not to add the dimension to a different
      \ref SUEntitiesRef.
@own{dimensions}
*/
SU_RESULT SUEntitiesAddDimensions(
    SUEntitiesRef entities_ref, size_t len, const SUDimensionRef dim_refs[]);
