 ...
 * 단순히 컴포넌트를 import하고 최소한의 props만 전달하면 됩니다.
 * 크기, 반응형 처리 등 모든 기능이 라이브러리 내부에서 자동으로 처리됩니다.
 */
export function CanvasViewerExample() {
  ...
  return (
    <div style={{ padding: isMobile ? 0 : 20, width: "100%" }}>
      {!isMobile && (
        <>
          <h3 style={{ fontSize: 24 }}>캔버스 뷰어 예제 (읽기 전용)</h3>
          <p
            style={{
              color: "#666",
              marginBottom: 16,
      ...
      {!isMobile && (
        ...
        >
          ...
          뷰어에서 실시간으로 볼 수 있습니다. 같은
          roomId("basic-example-room")를 사용하여 동기화됩니다.
        </div>
      )}
    </div>
  );
